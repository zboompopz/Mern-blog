...
export default class Users extends Component {
    ...
    submit(event, id) {
        ...
        if (id === 0) {
            axios.post(`http://localhost:8000/api/user/`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })

        } else {
            axios.put(`http://localhost:8000/api/user/${id}`, { name: this.state.name, email: this.state.email, password: this.state.password })
                .then(() => {
                    this.componentDidMount();
                })
        }
